...
# Ensure proper types
df = df.withColumn("mastodon_date", to_date(col("mastodon_date")))
...
#df = df.filter(col("cve_id").isNotNull() & col("mastodon_date").isNotNull())
...
# Save feature set
features_df.coalesce(1).write.mode("overwrite").parquet("features/features_mastodon.parquet")
