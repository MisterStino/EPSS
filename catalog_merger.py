...
# Load both CSV files
cve_df = pd.read_csv('catalogs_raw/cve_catalog.csv')
cve_df = cve_df[cve_df['state'] == 'PUBLISHED'] #remove rejected CVEs
exploit_db_df = pd.read_csv('catalogs_raw/exploitDB_catalog.csv')
kev_df = pd.read_csv('catalogs_raw/KEV_catalog.csv')
zdi_df = pd.read_csv('catalogs_raw/ZDI_catalog.csv')
...
# Merge CWE from KEV
merged_df['cwes'] = merged_df['cwes'].fillna(merged_df['KEV_cwes'])
merged_df.drop(columns=['KEV_cwes'], inplace=True)

# Merge CVSSv3_Base_Score values from ZDI catalog
merged_df['cvss_score'] = merged_df['cvss_score'].fillna(merged_df['ZDI_CVSS v3.0'])
merged_df.drop(columns=['ZDI_CVSS v3.0'], inplace=True)

# Merge vendors from KEV
...
merged_df.to_csv('enriched_catalog.csv', index=False)
