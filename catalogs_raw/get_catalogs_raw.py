        ...
        def process(file_path):
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    # Skip invalid/non-CVE files
                    if not isinstance(data, dict) or "cveMetadata" not in data:
                        return None

                    metadata = safe_dict(data.get("cveMetadata"))
                    containers = safe_dict(data.get("containers"))
                    cna = safe_dict(containers.get("cna"))

                    # CVSS parsing
                    cvss_score, cvss_version = "N/A", "N/A"
                    for metric in safe_list(cna.get("metrics")):
                        for key, value in safe_dict(metric).items():
                            if isinstance(value, dict) and "baseScore" in value:
                                cvss_score = value.get("baseScore", "N/A")
                    ...
                    cwes = "; ".join(
                        desc.get("cweId", "")
                        for pt in safe_list(cna.get("problemTypes"))
                        for desc in safe_list(pt.get("descriptions"))
                        if desc.get("lang") == "en"
                    )
                    ...
                    descriptions = "; ".join(
                        desc.get("value", "")
                        for desc in safe_list(cna.get("descriptions"))
                        if desc.get("lang") == "en"
                    )
                    ...
                    affected_products = []
                    vendors = []
                    for vendor in safe_list(cna.get("affected")):
                        vendor_name = vendor.get("vendor", "")
                        vendors.append(vendor_name)
                        for product_entry in safe_list(vendor.get("product")):
                            product_name = product_entry.get("product", "")
                            for version in safe_list(product_entry.get("versions")):
                                version_str = version.get("version", "")
                                if product_name or version_str:
                                    affected_products.append(f"{product_name} {version_str}".strip())
                    ...
                    return {
                        ...
                        "references": "; ".join(
                            ref.get("url", "")
                            for ref in safe_list(cna.get("references"))
                        ),
                        "tags": "; ".join(safe_list(cna.get("tags"))),
                        "solutions": "; ".join(
                            s.get("value", "")
                            for s in safe_list(cna.get("solutions"))
                            if s.get("lang") == "en"
                        ),
                        "workarounds": "; ".join(
                            w.get("value", "")
                            for w in safe_list(cna.get("workarounds"))
                            if w.get("lang") == "en"
                        ),
                        "credits": "; ".join(
                            c.get("value", "")
                            for c in safe_list(cna.get("credits"))
                            if c.get("lang") == "en"
                        ),
                        "exploits": "; ".join(
                            e.get("value", "")
                            for e in safe_list(cna.get("exploits"))
                            if e.get("lang") == "en"
                        ),
                        "source": cna.get("providerMetadata", {}).get("orgId", ""),
                        "date_recorded": cna.get("datePublic", "")
                    }
