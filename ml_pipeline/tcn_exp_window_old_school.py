...
LOCAL_CONFIG = {
    ...
    'tcn_filters': 64,     # TCN: number of filters per level
    'tcn_levels': 6,       # TCN: number of dilated blocks
    'tcn_kernel': 3,       # TCN: kernel size for convolutions
...
CLOUD_CONFIG = {
    ...
    'tcn_filters': 128,    # TCN: more filters for cloud GPU
    'tcn_levels': 8,       # TCN: deeper network for cloud
    'tcn_kernel': 3,       # TCN: kernel size for convolutions
...
# ───────────────────────────── NEW  Seq2SeqTCN ────────────────────────────
# Minimal, causal dilated-CNN forecaster. Returns the SAME shape as before: [B,L,H]
class Seq2SeqTCN(nn.Module):
    def __init__(self, n_num: int, n_bool: int, cat_sizes,
        ...
        # Categorical embeddings (identical to LSTM version)
        self.emb = nn.ModuleList([
            nn.Embedding(s, emb_dim, padding_idx=0) for s in cat_sizes
        ])
